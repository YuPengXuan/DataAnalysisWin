...
public class LostValueAction extends WindowAction {
	 ...
	 private boolean createDialog(){
		 ...
		 try{
			...
			dialog.setSize(new Dimension(230,180));
	 ...
	 private void deleteNullValueRow(){
		 ...
		 for(int i=0;i<chnColumnNameVec.size();i++){
             ...
             if(true == ConfigParser.columnInfoMap.get(enColumnName).mValueType.contains("VARCHAR")){
		 ...
		 if(true == DatabaseAction.Instance().deleteData(dataTable, condition.toString())){
	 ...
	 private void fillDefaultValueToNullRow(){
		 ...
		 try{
			 ...
			 for(int i=0;i<chnColumnNameVec.size();i++){
				 ...
				 DatabaseAction.Instance().updateColumn(tableName, columnNameVec, valueVec, condition);
