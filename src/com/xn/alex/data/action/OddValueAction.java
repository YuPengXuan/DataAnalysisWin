	...
	private Map<String, String> conditionMap = new ConcurrentHashMap<String, String>();
	 ...
	 public void takeAction(){
		 ...
		 if(null ==colName || 0 == colName.size()){
			 ...
			 JOptionPane.showMessageDialog(null,"请先选择要处理的数据列","错误信息",JOptionPane.ERROR_MESSAGE);			 
		     return;
		 }		 
	 ...
	 private boolean createChoiceDialog(){
		 ...
		 try{
		     ...
		     rightPanel.setBorder(BorderFactory.createEmptyBorder(30, 0, 30, 20));		     		    
	 ...
	 private void addListerner(){
		 ...
		 OKBt.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent event) {
				...
				Vector<String> selectedColumnVec = MainWindow.Instance().getSelectedEnColumnVec();
				...
				for(int i=0;i<selectedColumnVec.size();i++){
					...
					for (Map.Entry<String, String> entry : conditionMap.entrySet()) { 						
						...
						if(true == ConfigParser.columnInfoMap.get(columnName).mValueType.contains("VARCHAR")){
							
							condition =condition + columnName + entry.getKey() + "'" + entry.getValue() + "'";
						}
						else{
							condition = condition + columnName + entry.getKey() + entry.getValue();
						}
	 ...
	 private boolean isInputConditionValid(){
		 ...
		 if(largerChecker.isSelected()&& null!=largerCheckerTextField.getText()){
			 ...
			 if(false == isNum.matches()){				 
				 ...
				 JOptionPane.showMessageDialog(null,"请在大于条件中输入正确数字","错误信息",JOptionPane.ERROR_MESSAGE);
         ...
         if(lessChecker.isSelected()&& null!= lessCheckerTextField.getText()){
			 ...
			 if(false == isNum.matches()){
				 ...
				 JOptionPane.showMessageDialog(null,"请在小于条件中输入正确数字","错误信息",JOptionPane.ERROR_MESSAGE);
         ...
         if(largerChecker.isSelected()&&lessChecker.isSelected()){
        	 ...
        	 if(Double.parseDouble(lessNumberInStr)<=Double.parseDouble(largerNumInStr)){
        		 
        		 JOptionPane.showMessageDialog(null,"请输入输入正确范围数字","错误信息",JOptionPane.ERROR_MESSAGE);
         ...
         if(largerChecker.isSelected()&&lessChecker.isSelected()&&equalChecker.isSelected()){
        	 
        	 JOptionPane.showMessageDialog(null,"三个条件不能同时选择","错误信息",JOptionPane.ERROR_MESSAGE);
         ...
         if(largerChecker.isSelected()&&equalChecker.isSelected()){
        	 if(Double.parseDouble(largerNumInStr)!=Double.parseDouble(equalInStr)){
        		 JOptionPane.showMessageDialog(null,"大于和等于数字必须相同","错误信息",JOptionPane.ERROR_MESSAGE);
         ...
         if(lessChecker.isSelected()&&equalChecker.isSelected()){
        	 if(Double.parseDouble(lessNumberInStr)!=Double.parseDouble(equalInStr)){
        		 JOptionPane.showMessageDialog(null,"小于和等于数字必须相同","错误信息",JOptionPane.ERROR_MESSAGE);
