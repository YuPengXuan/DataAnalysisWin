...
public class ProfitModelAction5 extends ProfitModelInterface implements ActionListener{
	 ...
	 public void takeAction(){
		...
		//利润模型界面
	 	public void createWindow(){
		    ...
		    JLabel title = new JLabel("APP客户保有利润分析",JLabel.CENTER);
		    ...
		    ImageIcon imageicon1 =new ImageIcon(ClassLoader.getSystemResource("images/APP客户保有.png"));
		    ...
		    JLabel a = new JLabel("N:APP客户总数"+"--"+"V:APP客户价值"+"--"+"c:挽留激励成本"
		    +"--"+"γ:挽留成功率");
		    ...
		    JLabel b = new JLabel("α:APP易流失客户占比"+"--"+"F0:模型命中率"
		    +"--"+"F1:模型误判率--F2:模型成功率");
		    ...
		    input_K = new JTextField(""+numTotal,5);
		 ...
		 private double executeAlogrithm(double paraA, double paraB, double paraC,double paraD,double paraE,double paraF,double paraG,double paraH,double paraI,double paraJ){
			 ...
			 for(int i = 0;i<m_resultObjVec.size();i++){
				 ...
				 if(y>=paraE && y<=paraF && x>= paraG && x<=paraH && succRate>=paraI && succRate<= paraJ){
					 ...
					 P = ( paraK* alfa* y *paraA* (paraB-paraC)) - (paraK* (1-alfa)* x* paraC);
