...
public class ProfitModelAction2 extends ProfitModelInterface implements ActionListener{
	...
	private JTextField input_E;
	
	private JTextField input_F;
	
	private JTextField input_G;
	
	private JTextField input_H;
	
	private JTextField input_I;
	
	private JTextField input_J;
	
	private JTextField input_K;
	
	private JTextField input_L;
	
	private JButton okBt;
	
	private double paraA = -1;//V
	
	private double paraB = -1;//c
	
	private double paraC = -1;//d
	
	private double paraD = -1;//b
	
	private double paraE = -1;//F0
	
	private double paraF = -1;//F0
	
	private double paraG = -1;//F1
	
	private double paraH = -1;//F1
	
	private double paraI = -1;//F2
	
	private double paraJ = -1;//F2
	
	private double paraK = -1;//N
	
	private double paraL = -1;//营销人群数
	...
	public ProfitModelAction2(){
	 ...
	 public void takeAction(){
		...
		public void createWindow(){
		    ...
		    JLabel title = new JLabel("4G外呼营销利润分析",JLabel.CENTER);
		     
		    title.setFont(new java.awt.Font("Dialog", 1, 23));
		    ...
		    ImageIcon imageicon1 =new ImageIcon(ClassLoader.getSystemResource("images/4G外呼营销.png"));
		    ...
		    JLabel a = new JLabel("N：客户总数--α：历史成功营销客户占比--V：产品单价--c：营销优惠");
		    
		    a.setFont(new java.awt.Font("Dialog", Font.ITALIC, 13));

		    topPanel.add(a);
		    
		    JLabel b = new JLabel("d：外呼接触成本--b：外呼打扰代价--F0：模型命中率--F1：模型误判率--F2：模型成功率");
		    ...
		    centerPanel.setBorder(BorderFactory.createEmptyBorder(20, 30, 0, 30));
		    ...
		    //
		    /*
		    JLabel parameter2 = new JLabel("4、模型限制条件",JLabel.LEFT);
		    
		    parameter2.setFont(new java.awt.Font("Dialog", 1, 16));
		     
		    centerPanel.add(parameter2);*/
		 ...
		 private boolean isValueValid(){
			 ...
			 paraK = Integer.parseInt(paraKText);
			 ...
			 if("".equals(paraAText) || Double.parseDouble(paraAText)<0.0){
			 ...
			 if("".equals(paraDText) || Double.parseDouble(paraDText)<0){
				 ...
				 JOptionPane.showMessageDialog(null,"请输入正确的参数b","错误信息",JOptionPane.ERROR_MESSAGE);
		 ...
		 private double executeAlogrithm(double paraA, double paraB, double paraC,double paraD,double paraE,double paraF,double paraG,double paraH,double paraI,double paraJ){
			 ...
			 for(int i = 0;i<m_resultObjVec.size();i++){
				 ...
				 if(y>=paraE && y<=paraF && x>= paraG && x<=paraH && succRate>=paraI && succRate<= paraJ){
					 ...
					 P = ( paraK* alfa* y *(paraA-paraB-paraC)) - (paraK* (1-alfa)* x* (paraD+paraC));
			 ...
			 Collections.sort(m_pResultVec, new ProfitResultNode());
